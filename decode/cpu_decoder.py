...
#!/usr/bin/env python3
"""
Image Decoder Script using VC-6 CPU Codec.

This script decodes images in a given directory (or a single file) into raw RGB format.
It uses the V-Nova VC-6 codec with CPU backend for decoding.
...
def decode_images(image_list: List[str], max_width: int, max_height: int, batch: int, loq: int, dst_dir: str) -> None:
    ...
    try:
        ...
        # Initialise a batch decoder with CPU backend with max height and max width and CPU memory type.
        vc6decoder = vc6codec.BatchDecoder(max_width, max_height, vc6codec.CodecBackendType.CPU, vc6codec.PictureFormat.RGB_8, vc6codec.ImageMemoryType.CPU, False, batch, num_images)
